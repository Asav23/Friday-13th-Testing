 ...
 * the 13th based on the calendar specifications.5
 *
...
// Name should be changed to something less ambigous e.g calendar day

class Day {
    private int dayNumber; // The day number in the month  to find 13th so when 5 == 13th   
...
class Calendar {
    private int totalDays; // no getter for total days
    ...
    public Calendar(int totalDays2, int numberOfMonths2, int i) {
    ...
    private void validateInput(int[] daysInMonths) {
        ...
        }    /// days in months doesnt chewck fir negative value 
        if (numberOfMonths <= 0) {
...
/////////////////////////// for Testing purposes
    public int getStartDayOfMonth(int monthIndex) {
        int currentDayOfWeek = 0; // Start with Sunday for the first month (index 0)
        
        // Loop through all months up to the given month index
        for (int i = 0; i < monthIndex; i++) {
            // Update the day of the week by adding the number of days in the current month
            currentDayOfWeek = (currentDayOfWeek + months[i].getDaysInMonth()) % 7;
        }
        
        return currentDayOfWeek; // Return the calculated start day of the specified month
    }
...
///////////////////////// handling for input should be in main class/ shouldnt accept faulty input at all
    ...
    System.out.println("You entered " + totalDays);   /// poor error handling 

        // Read the number of months in the year
        System.out.print("Enter the number of months in the year: ");
        int numberOfMonths = scanner.nextInt();
        System.out.println("You entered " + numberOfMonths);   /// poor error handling   still takes input error handling expected


// code doesnt include 
        // Read the number of days in each month
        int[] daysInMonths = new int[numberOfMonths];
        System.out.print("Enter the number of days in each month:");
        for (int i = 0; i < numberOfMonths; i++) {
            daysInMonths[i] = scanner.nextInt();
            System.out.println("You entered " + daysInMonths[i] + " days for month " + (i + 1) + ".");
        ...
        try {
            ...
            System.out.print("The number of Fridays that fall on the 13th is: " + friday13Count);
